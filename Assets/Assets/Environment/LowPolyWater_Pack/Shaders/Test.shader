...
Properties { 
	...
	_ReflectionTex ("Reflection Texture", 2D) = "black" {} // Custom environment texture for reflection
}
...
CGINCLUDE 
	...
	sampler2D _ShoreTex;
	sampler2D _ReflectionTex; // Custom reflection texture
	...
	v2f vert(appdata_full v)
	{
        ...
        o.worldRefl = reflect(-worldViewDir, worldNormal); // Calculate reflection vector
	...
	{ 
		...
		// Reflection calculation (without reflection probe)
		half4 reflectionColor = tex2D(_ReflectionTex, i.worldRefl.xy); // Sample custom environment texture
		baseColor.rgb = lerp(baseColor.rgb, reflectionColor.rgb, 0.5); // Blend base color with reflection
